...
String.prototype.isAnagramOf = function (candidate: string): boolean {
  ...
  // O(nlogn) or O(n), depending. Assume O(nlogn).
...
String.prototype.isUnicodeAnagramOf = function (candidate: string): boolean {
  ...
  for (let i: number = 0; i < self.length; i++) {
    const character: string = self[i]
    if (character === ' ') continue

    typeof instances[character] === 'number'
      ? instances[character]++
      : (instances[character] = 0)
  }

  for (let i: number = 0; i < string.length; i++) {
    const character: string = string[i]
    if (character === ' ') continue

    if (instances[character] === void 0) return false

    instances[character] === 0
      ? delete instances[character]
      : instances[character]--
  }

  // O(n) or O(nlogn) - depending :p (https://stackoverflow.com/questions/7716812/object-keys-complexity) probably O(nlogn)
  return Object.keys(instances).length === 0
}
...
String.prototype.isAnagramOf2 = function (candidate: string): boolean {
  ...
  for (let i = 0; i < self.length; i++) {
    const [lowerCaseSelf, lowerCaseCandidate] = [self[i], string[i]]
